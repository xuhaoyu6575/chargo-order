...
const C = {
  bgPrimary: '050f1e', bgCard: '0b1f38', bgDeep: '020c18',
  border: '1a3a5c', textPrimary: 'e2e8f0', textSecondary: '94a3b8', textDim: '64748b',
  cyan: '00d4ff', blue: '3b82f6', purple: 'a855f7', green: '10b981', orange: 'f97316', red: 'ef4444',
  white: 'ffffff', accentBar: '0066aa',
};
...
async function createAssets() {
  const bgSvg = `<svg xmlns="http://www.w3.org/2000/svg" width="1440" height="810">
    ...
    <defs><pattern id="g" width="50" height="50" patternUnits="userSpaceOnUse">
      <path d="M50 0H0v50" fill="none" stroke="rgba(26,58,92,0.12)" stroke-width="1"/>
    </pattern></defs>
  ...
  const titleBgSvg = `<svg xmlns="http://www.w3.org/2000/svg" width="1440" height="810">
    <rect width="100%" height="100%" fill="#${C.bgPrimary}"/>
    <defs><pattern id="g" width="50" height="50" patternUnits="userSpaceOnUse">
      <path d="M50 0H0v50" fill="none" stroke="rgba(26,58,92,0.15)" stroke-width="1"/>
    </pattern>
    <radialGradient id="r" cx="50%" cy="40%"><stop offset="0%" stop-color="rgba(0,212,255,0.06)"/><stop offset="100%" stop-color="transparent"/></radialGradient>
    </defs>
    <rect width="100%" height="100%" fill="url(#g)"/>
  ...
  const accentSvg = `<svg xmlns="http://www.w3.org/2000/svg" width="1440" height="6">
    <defs><linearGradient id="a" x1="0%" x2="100%"><stop offset="0%" stop-color="#${C.cyan}"/><stop offset="50%" stop-color="#${C.blue}"/><stop offset="80%" stop-color="#${C.purple}"/><stop offset="100%" stop-color="transparent"/></linearGradient></defs>
...
.card { background: #${C.bgCard}; border: 1px solid #${C.border}; border-radius: 6pt; padding: 12pt; }
.card-glow { background: #${C.bgCard}; border: 1px solid #${C.cyan}; border-radius: 6pt; padding: 12pt; }
.sub-box { background: #${C.bgDeep}; border: 1px solid #${C.border}; border-radius: 4pt; padding: 8pt; margin-top: 4pt; }
.badge { display: inline-block; font-size: 8pt; font-weight: 700; padding: 2pt 8pt; border-radius: 3pt; text-transform: uppercase; }
.badge-cyan { color: #4fc3f7; }
.badge-orange { color: #fbbf24; }
.badge-purple { color: #a78bfa; }
.badge-bg-cyan { background: rgba(0,122,204,0.2); border: 1px solid rgba(0,122,204,0.5); padding: 2pt 8pt; border-radius: 3pt; display: inline-block; }
.badge-bg-orange { background: rgba(217,119,6,0.2); border: 1px solid rgba(217,119,6,0.5); padding: 2pt 8pt; border-radius: 3pt; display: inline-block; }
.badge-bg-purple { background: rgba(167,139,250,0.15); border: 1px solid rgba(167,139,250,0.4); padding: 2pt 8pt; border-radius: 3pt; display: inline-block; }
.tag-wrap { display: inline-block; padding: 1pt 6pt; border-radius: 10pt; border: 1px solid currentColor; margin-right: 4pt; }
.tag { font-size: 8pt; font-weight: 600; }
.section-num { position: absolute; top: 8pt; right: 40pt; font-size: 48pt; font-weight: 800; color: rgba(0,212,255,0.05); font-family: Impact, Arial, sans-serif; }
.stat-num { font-size: 28pt; font-weight: 800; font-family: Impact, Arial, sans-serif; }
...
function slideHTML(bodyClass, innerContent) {
  ...
  html = html.replace(/<p class="tiny" style="background:#([^;]+);color:#fff;padding:([^;]+);border-radius:([^;]+);font-weight:700">GET<\/p>/g,
    (_, bg, pad, rad) => `<div style="background:#${bg};padding:${pad};border-radius:${rad};display:inline-block"><p class="tiny" style="color:#fff;font-weight:700">GET</p></div>`);
...
async function buildPptx() {
  ...
  const outPath = path.join(WORKSPACE, '..', 'AI研发提效总结_v2.pptx');
